...
# WorkflowSpec: reusable CHOICE metadata and degree-specific inequality specs.
...
if marker not in text:
    raise SystemExit('inequality marker not found')
...
# WorkflowSession: initialize, display and cycle CHOICE fields without exposing codes.
...
# Machine: choice cells consume direction keys and never load raw relation codes into tokens.
...
if route_marker not in text:
    raise SystemExit('machine choice route marker not found')
...
# Android: display relation symbols instead of evaluator codes.
...
# Regression tests: degree shape, choice cycling, serialization and engine delegation.
...
'''
if marker not in text:
    raise SystemExit('ineq spec marker not found')
...
'''
if marker not in text:
    raise SystemExit('ineq session marker not found')
text = text.replace(marker, method + marker, 1)
text = replace_once(text,
