...
public final class Cw991LayoutMetrics {
    ...
    public static KeyVisual forKey(Role role, float touchWidth, float touchHeight,
        ...
        switch (role) {
            case NUMBER -> {
                diameter = diameter(width, height, viewport, 0.86f, 0.90f, 0.150f);
            ...
            case FUNCTION -> {
                diameter = diameter(width, height, viewport, 0.84f, 0.84f, 0.122f);
                mainRatio = 0.56f;
                secondaryRatio = 0.39f;
                verticalBias = 0.50f;
            }
            case CONTEXT, SHIFT -> {
                diameter = diameter(width, height, viewport, 0.74f, 0.88f, 0.116f);
                mainRatio = 0.50f;
